...
class BoardSpec extends Specification with Mockito with Tables {
  ...
  "hasWinner" should {
    "false when a line has 3-sequenced symbol" in {
      "patterns(i,j,p)" | "ret" |
        ...
        // black
        Seq((0, 0, Black), (0, 1, Black), (0, 2, Black)) ! true |
        // black
        Seq((0, 0, Black), (0, 1, Black), (0, 2, Black), (1, 1, White)) ! true |> { (score, ret) =>
        val board = play(score)
        board.hasWinner must beEqualTo(ret)
      }
    }
  ...
  "isDraw" should {
    "true when no winner and board is filled" in {
      val score = Seq(
        (0, 0, Black),
        (0, 1, Black),
        (0, 2, White),
      ...
      val board = play(score)
      board.isDraw must beEqualTo(true)
    }
    "false when" in {
      val score = Seq(
        (0, 0, White),
      ...
      val board = play(score)
      board.isDraw must beEqualTo(false)
    }
  }
